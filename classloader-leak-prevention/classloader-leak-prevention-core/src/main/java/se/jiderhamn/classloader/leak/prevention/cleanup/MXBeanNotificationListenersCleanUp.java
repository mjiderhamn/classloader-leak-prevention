...
public class MXBeanNotificationListenersCleanUp implements ClassLoaderPreMortemCleanUp {
  ...
  public void cleanUp(ClassLoaderLeakPreventor preventor) {
    ...
    final Class<?> listenerWrapperClass = preventor
        .findClass("com.sun.jmx.interceptor.DefaultMBeanServerInterceptor$ListenerWrapper");

    final boolean canProcessNotificationEmitterSupport = listenerListField != null && listenerInfoClass != null
      ...
      for (Object mxBean : ManagementFactory.getPlatformMXBeans(platformInterface)) {
        if (mxBean instanceof NotificationEmitter) { // The MXBean may have NotificationListeners
          if (canProcessNotificationEmitterSupport
            ...
            final List<? /* NotificationEmitterSupport.ListenerInfo */> listenerList = preventor
                .getFieldValue(listenerListField, mxBean);
            if (listenerList != null) {
              for (Object listenerInfo : listenerList) { // Loop all listeners
                ...
                if (preventor.isLoadedInClassLoader(rawListener) || preventor.isLoadedInClassLoader(filter)
                    || preventor.isLoadedInClassLoader(handback)) {
                  preventor.warn(((listener == rawListener) ? "Listener '" : "Wrapped listener '") + listener
                      + "' (or its filter or handback) of MXBean " + mxBean + " of interface " + platformInterface
